    ...
    try {
      ...
      command match {
        ...
        case "code-filter" =>
          ...
          val query: String = s"SELECT count(*) FROM log_lines WHERE response = $responseCode"
        ...
        case "time-filter" =>
          ...
          val query: String = s"SELECT count(*) FROM log_lines WHERE time BETWEEN $from AND $to"
          // TODO 11: write SQL query to filter view `log_lines` by `time` between `from` and `to`
          val count: Long = spark.sql(query).first().getAs[Long](0)
        ...
        case "count-by-code" =>
          ...
          println("Code,Count")
          val query: String = "SELECT response, count(*) AS count FROM log_lines GROUP BY response ORDER BY response"
        ...
        case "sum-bytes-by-code" =>
          ...
          val query: String = "SELECT response, SUM(bytes) FROM log_lines GROUP BY response ORDER BY response"
        ...
        case "avg-bytes-by-code" =>
          ...
          val query: String = "SELECT response, AVG(bytes) AS avg_bytes FROM log_lines GROUP BY response ORDER BY response"
        ...
        case "top-host" =>
          ...
          val query: String = "SELECT host, COUNT(*) AS entries_count FROM log_lines GROUP BY host ORDER BY entries_count DESC LIMIT 1"
